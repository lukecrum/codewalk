...
`);
    ...
    for (let i = 1;i < lines.length; i++) {
      ...
      if (line.startsWith("@@")) {
        ...
        if (line.startsWith("Binary files") || line.startsWith("index ") || line.startsWith("---") || line.startsWith("+++") || line.startsWith("new file") || line.startsWith("deleted file")) {
          continue;
        }
  ...
  for (const tc of trackedCommits) {
    ...
    for (const change of tc.tracking.changes) {
      ...
      for (const fileChange of change.files) {
        ...
        if (!fileDiff)
          continue;
        const selectedHunks = fileChange.hunks.map((hunkNum) => fileDiff.hunks.find((h2) => h2.hunkNumber === hunkNum)).filter((h2) => h2 != null);
        if (selectedHunks.length === 0)
          continue;
        const existingFile = group.files.find((f) => f.path === fileChange.path);
        if (existingFile) {
          for (const hunk of selectedHunks) {
            if (!existingFile.hunks.find((h2) => h2.hunkNumber === hunk.hunkNumber)) {
  ...
  scrollToSelected() {
    ...
    for (const reasoningBox of contentChildren) {
      ...
      if (item && item.type === "reasoning" && item.reasoningIdx === selectedItem.reasoningIdx) {
        ...
        const reasoningChildren = reasoningBox.getChildren();
        let fileOffset = reasoningChildren[0]?.height || 2;
